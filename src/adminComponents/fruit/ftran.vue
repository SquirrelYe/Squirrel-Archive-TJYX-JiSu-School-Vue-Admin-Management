<template>
  <div class="container">
    <div class="row">
      <div class="col-sm-12">
        <h4 class="pull-left page-title">水果订单列表管理</h4>
      </div>
    </div>
        ...
        <div class="panel panel-default">
          ...
          <div class="row">
            <form class="form-horizontal" role="form">
              <div class="col-md-3">
                <div class="col-md-4 control-label"><strong>水果项目：</strong></div>
                <div class="col-md-8">
                  <select class="form-control" v-model="fitem_id" @change="getTranByFruit()">
                    <option v-for="(item, index) in fitem" :key="index" :value="item.id">{{ item.name }}</option>
                  </select>
                </div>
              </div>
            </form>
          ...
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <!-- <input type="text" class="form-control search-bar" placeholder="输入搜索水果订单列表名称" v-model="searchkey" v-on:keyup.enter="search()"> -->
                  ...
                  <table class="table table-bordered table-striped table-hover" style id="datatable-editable">
                    ...
                    <tbody v-if="showItem">
                      <tr class="gradeX" v-for="(item, index) in showItem.rows" :key="index" :class="item.condition == 1 ? 'text-success' : ''">
                        ...
                        <td v-else></td>
                        <td v-if="item.location">{{ item.location.phone }}</td>
                        <td v-else></td>
                        <td v-if="item.location">{{ item.location.school }}</td>
                        <td v-else></td>
                        <td v-if="item.location">{{ item.location.dom }}</td>
                        <td v-else></td>
                        <td v-if="item.location">{{ item.location.detail }}</td>
                        <td v-else></td>
...
export default {
  ...
  methods: {
    ...
    // 获取所有水果
    getAllfitem() {
      apis.fitem.findAllBySchool(this.school_id, 0, 1000).then(res => {
        this.fitem = res.data.rows;
        print.log('所有水果', res.data);
      });
    },
    getTranByFruit() {
      this.getAllFtranByKind(this.fitem_id);
      this.offsize = 0;
    },
    // 查询订单
    getAllFtranByKind(fitem_id) {
      apis.cart.findByFruit(fitem_id, this.offsize, this.limit).then(res => {
        this.showItem = res.data;
        print.log('订单', res.data);
      });
    },
    // 更改订单物流状态
    editItem(item) {
    ...
    toEdit() {
      apis.cart.updateJudge(this.currentItem.id, this.currentItem.judgec).then(res => {
