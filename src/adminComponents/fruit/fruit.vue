...
export default {
  ...
  methods: {
    ...
    findAndCountAll(offsize, limit) {
      apis.fruit.findAllBySchool(this.school_id, offsize, limit).then(res => {
    ...
    toCreat() {
      ...
      if (this.name != null && this.condition != null) {
        apis.fruit.creat(this.name, this.condition, this.school_id).then(res => {
    ...
    toEdit() {
      ...
      if (this.eitem != null) {
        apis.fruit.update(this.eitem.id, this.eitem.title, this.eitem.condition).then(res => {
          if (res.data[0]) {
    ...
    DeleteItem(item) {
      if (confirm(`你确定要删除${item.title}？`)) {
        apis.fruit.delete(item.id).then(res => {
    ...
    search() {
      if (this.searchkey)
        apis.fruit.findAndCountAllLikeByNameSchool(this.searchkey, this.school_id).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    }
