...
export default {
  ...
  methods: {
    ...
    // 获取一级菜单
    findFirstMenu() {
      apis.fruit.findAllBySchool(this.school_id, 0, 100).then(res => {
        this.firstItems = res.data.rows;
        console.log('水果一级菜单', res.data);
      });
    },
    // 获取所有水果二级菜单
    findAndCountAll(offsize, limit) {
      apis.mfruit.findAllBySchool(this.school_id, offsize, limit).then(res => {
        this.showItem = res.data;
        console.log('水果二级菜单', res.data);
      });
    },
    creat() {
      this.judge = 0;
    ...
    toCreat() {
      ...
      if (this.first != null && this.name != null && this.condition != null) {
        apis.mfruit.creat(this.first, this.name, this.condition, this.school_id).then(res => {
    ...
    toEdit() {
      ...
      if (this.eitem != null) {
        apis.mfruit.update(this.eitem.id, this.eitem.fruit_id, this.eitem.title, this.eitem.condition).then(res => {
          if (res.data[0]) {
    ...
    DeleteItem(item) {
      if (confirm(`你确定要删除${item.title}？`)) {
        apis.mfruit.delete(item.id).then(res => {
    ...
    search() {
      if (this.searchkey)
        apis.mfruit.findAndCountAllLikeByNameSchool(this.searchkey, this.school_id).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    }
