                  ...
                  <table style="table-layout: fixed" class="table table-bordered table-striped table-hover" id="datatable-editable">
                    ...
                    <tbody v-if="showItem">
                      <tr class="gradeX" v-for="(item, index) in showItem.rows" :key="index" :class="item.condition == -1 ? 'text-danger' : ''">
                        ...
                        <td>FI-{{ item.id }}</td>
                        <td v-if="item.mfruit">{{ item.mfruit.title }}</td>
                        <td v-else></td>
...
export default {
  ...
  methods: {
    ...
    findAndCountAll(offsize, limit) {
      apis.fitem.findAllBySchool(this.school_id, offsize, limit).then(res => {
        this.showItem = res.data;
      });
    },
    // 获取水果二级分类
    findAllFruit() {
      apis.mfruit.findAllBySchool(this.school_id, 0, 100).then(res => {
        this.secondItems = res.data.rows;
        console.log('水果二级分类', res.data);
      });
    },
    // 更新水果状态
    ...
    toCreat() {
      ...
      if (this.name != null && this.title != null && this.price != null && this.detail != null && this.mfruit_id != null && this.condition != null && this.logo != null) {
        apis.fitem.creat(this.logo, this.name, this.title, this.price, this.detail, this.mfruit_id, this.condition, this.school_id).then(res => {
    ...
    toEdit() {
      ...
      if (this.currentItem != null) {
        ...
        apis.fitem.update(item.id, item.logo, item.name, item.title, item.price, item.detail, item.mfruit_id, item.condition).then(res => {
          if (res.data[0]) {
    ...
    DeleteItem(item) {
      if (confirm(`你确定要删除${item.title}？`)) {
        apis.fitem.delete(item.id).then(res => {
    ...
    search() {
      if (this.searchkey)
        apis.fitem.findAndCountAllLikeByNameSchool(this.searchkey, this.school_id).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    },
