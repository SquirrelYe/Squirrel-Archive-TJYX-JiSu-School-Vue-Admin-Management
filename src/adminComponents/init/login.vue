        ...
        <div class="panel-body">
          <div class="form-horizontal m-t-20">
            <div class="form-group">
              <div class="col-xs-12">
                <input class="form-control input-lg" type="text" required placeholder="用户名" v-model="name" v-on:keyup.enter="login()" />
              </div>
            </div>
            ...
            <div class="form-group m-t-30">
              <div class="col-sm-7">
                <a href="javascript:void(0)" @click="confirmadmin()"> <i class="fa fa-lock m-r-5"></i> 忘记密码？ </a>
              </div>
  ...
  methods: {
    ...
    login() {
      if (this.name == null || this.pass == null) {
        ...
        apis.user.login(this.name, this.pass).then(res => {
          if (res) {
            print.log(res.data);
            if (res.data.type == 1) {
    ...
    test() {
      for (let i = 0; i < 50; i++) {
        apis.user.findAndCountAll(0, 10).then(res => {
          print.log(res.data);
        });
      }
    }
