...
export default {
  ...
  methods: {
    ...
    findAndCountAll(offsize, limit) {
      apis.card.findAndCountAllBySchool(this.school_id, offsize, limit).then(res => {
        this.showItem = res.data;
        print.log('开卡列表', res.data);
      });
    },
    ...
    DeleteItem(item) {
      if (confirm(`你确定要删除${item.location.name}？`)) {
        apis.card.delete(item.id).then(res => {
    ...
    setOrder(item) {
      this.item = item;
      apis.user.findAndCountAllXYDS(2, this.school_id, 0, 1000).then(res => {
        this.takes = res.data.rows;
        print.log('校园大使', res.data.rows);
      });
    },
    // 派发
    send() {
      ...
      //  发送到校园大使
      apis.card.setTaken(this.item.id, this.take, 1).then(res => {
        if (res.data[0] == 1) {
    ...
    search() {
      if (this.searchkey)
        apis.card.findAndCountAllLikeByNameSchool(this.searchkey, this.school_id).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    },
    ...
    creatCard() {
      print.log(this.card);
      let c = this.card;
      apis.card.create(null, 0, c.name, c.price, c.phone, this.school_id, c.dom, c.detail).then(res => {
        print.log(res.data);
