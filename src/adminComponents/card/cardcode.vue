                  ...
                  <table style="table-layout: fixed" class="table table-bordered table-striped table-hover" id="datatable-editable">
                    ...
                    <tbody v-if="showItem">
                      <tr class="gradeX" v-for="(item, index) in showItem.rows" :key="index" :class="item.condition == -1 ? 'text-danger' : ''">
                        ...
                        <td>EI-{{ item.id }}</td>
                        <td v-if="item.exam">{{ item.exam.title }}</td>
                        <td v-else></td>
...
export default {
  ...
  methods: {
    ...
    findAndCountAll(offsize, limit) {
      apis.eitem.findAndCountAll(offsize, limit).then(res => {
    ...
    toEdit() {
      ...
      if (this.currentItem != null) {
        ...
        apis.eitem.update(item.id, item.logo, item.name, item.title, item.price, item.detail, item.exam_id, item.condition).then(res => {
          if (res.data[0]) {
