        ...
        <div class="modal-content">
          ...
          <div class="modal-body" align="center" v-if="judge == 0 && item">
            ...
            <address class="ng-scope" v-if="item.location">
              <strong>学校:{{ item.location.school }}</strong
              ><br />
              <strong>宿舍:{{ item.location.dom }}</strong
              ><br />
              <strong>姓名:{{ item.location.name }}</strong
              ><br />
              <strong>电话:{{ item.location.phone }}</strong
              ><br /><br />
              <hr />
            </address>
          </div>
          ...
          <div class="modal-body" align="center" v-if="judge == 1 && item">
            <h4>查看收货地址</h4>
            <strong>收货地址:{{ item.arrive }}</strong
            ><br />
          </div>
...
export default {
  ...
  methods: {
    ...
    findAndCountAll(offsize, limit) {
      apis.lsend.findAllBySchool(this.school_id, offsize, limit).then(res => {
    ...
    toTake() {
      ...
      apis.lsend.updateTake(this.item.id, this.take).then(res => {
        ...
        if (res.data[0] == 1) {
          //  更改lsend状态,已接单
          apis.lsend.update(this.item.id, 1).then(res => {
    ...
    toCode() {
      ...
      apis.lsend.updateCode(this.item.id, this.code, 4).then(res => {
    ...
    search() {
      if (this.searchkey)
        apis.lsend.findAndCountAllLikeByCusName(this.searchkey, this.school_id, 0, 100).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    }
