...
export default {
  ...
  methods: {
    ...
    getAllUser(offsize, limit) {
      apis.user.findAndCountAllByTypeSchool(0, this.school_id, offsize, limit).then(res => {
    ...
    search() {
      if (this.searchkey)
        apis.user.findAndCountAllByTypeLikeByNameSchool(0, this.searchkey, this.school_id).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    }
