          ...
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <input type="text" class="form-control search-bar" placeholder="输入搜索校园大使用户名" v-model="searchkey" v-on:keyup.enter="search()" />
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body" align="center" v-if="judge == 2 && stock">
            <h4>校园大使资产信息</h4>
            <address class="ng-scope"><strong>余额:</strong>{{ stock.money }}<br /></address>
          </div>
...
export default {
  ...
  methods: {
    ...page,
    init() {
      this.getAllUser(this.offsize, this.limit);
    },
    // 获取所有参赛者
    getAllUser(offsize, limit) {
      apis.user.findAndCountAllXYDS(2, this.school_id, offsize, limit).then(res => {
        this.showItem = res.data;
      });
    },

    getWX(item) {
      this.wxinfo = item.info;
      this.judge = 0;
    },
    getAuthen(item) {
      this.authen = item.authen;
      this.judge = 1;
    },
    getStock(item) {
    ...
    DeleteItem(item) {
      if (confirm(`你确定要删除${item.name}？`)) {
        apis.user.delete(item.id).then(res => {
    ...
    search() {
      if (this.searchkey)
        apis.user.findAndCountAllXYDSByNameSchool(2, this.school_id, this.searchkey).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    }
