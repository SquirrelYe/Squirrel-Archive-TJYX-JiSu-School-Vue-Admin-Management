...
export default {
  ...
  methods: {
    ...
    getAllUser(offsize, limit) {
      apis.user.findAndCountAllByTypeSchool(1, this.school_id, offsize, limit).then(res => {
    ...
    search() {
      if (this.searchkey)
        apis.user.findAndCountAllByTypeLikeByNameSchool(1, this.searchkey, this.school_id).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    },
    // 新建管理员
    creatAdmin() {
      apis.user.creatAdmin(this.name, this.pass, this.mail, this.phone, this.school_id).then(res => {
        if (res.data[1]) {
