<template>
  <div class="container">
    <div class="row">
      <div class="col-sm-12">
        <h4 class="pull-left page-title">考试内容项目管理</h4>
      </div>
    </div>
      ...
      <div class="col-md-12">
        <div class="panel panel-default">
          <div class="panel-heading">
            <h3 class="panel-title">极速校园后台管理系统-考试项目管理</h3>
          </div>
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <button class="btn btn-primary waves-effect waves-light" data-toggle="modal" data-target="#Model" @click="creat()">添加考试项目<i class="fa fa-plus"></i></button>
                <input type="text" class="form-control search-bar" placeholder="输入搜索考试项目名称" v-model="searchkey" v-on:keyup.enter="search()" />
                  ...
                  <table style="table-layout: fixed" class="table table-bordered table-striped table-hover" id="datatable-editable">
                    ...
                    <tbody v-if="showItem">
                      <tr class="gradeX" v-for="(item, index) in showItem.rows" :key="index" :class="item.condition == -1 ? 'text-danger' : ''">
                        ...
                        <td>EI-{{ item.id }}</td>
                        <td v-if="item.mexam">{{ item.mexam.title }}</td>
                        <td v-else></td>
      ...
      <div class="modal-dialog" style="width: 55%">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel" v-if="judge == 0">新建考试项目信息</h4>
            <h4 class="modal-title" id="myModalLabel" v-if="judge == 1">修改考试项目信息</h4>
          </div>
                  ...
                  <div class="panel-body">
                    ...
                    <form class="form-horizontal" role="form" v-if="judge == 0 && secondItems">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-10">
                          <input type="file" id="file1" class="form-control" value="上传图片" @change="selectDetail" accept="image/gif,image/jpeg,image/jpg,image/png,image/svg" />
                        </div>
                      </div>
                    </form>
                    <!-- 编辑 -->
...
export default {
  ...
  methods: {
    ...
    findAndCountAll(offsize, limit) {
      apis.eitem.findAllBySchool(this.school_id, offsize, limit).then(res => {
        this.showItem = res.data;
        print.log('考试项目', res.data);
      });
    },
    // 获取考试二级分类
    findAllExam() {
      apis.mexam.findAllBySchool(this.school_id, 0, 100).then(res => {
        this.secondItems = res.data.rows;
        print.log('考试二级分类', res.data);
      });
    },
    // 更新状态
    ...
    toCreat() {
      ...
      if (this.name != null && this.title != null && this.price != null && this.detail != null && this.mexam_id != null && this.condition != null && this.logo != null) {
        apis.eitem.creat(this.logo, this.name, this.title, this.price, this.detail, this.mexam_id, this.condition, this.school_id).then(res => {
    ...
    toEdit() {
      ...
      if (this.currentItem != null) {
        ...
        apis.eitem.update(item.id, item.logo, item.name, item.title, item.price, item.detail, item.mexam_id, item.condition).then(res => {
          if (res.data[0]) {
    ...
    DeleteItem(item) {
      if (confirm(`你确定要删除${item.title}？`)) {
        apis.eitem.delete(item.id).then(res => {
          s_alert.Success('删除成功!', '成功移除了一个考试项目', 'success');
          this.init();
        });
      }
    },
    ...
    search() {
      if (this.searchkey)
        apis.eitem.findAndCountAllLikeByNameSchool(this.searchkey, this.school_id).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    },
