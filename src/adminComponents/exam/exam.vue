...
export default {
  ...
  methods: {
    ...
    // 获取所有考试一级菜单
    findAndCountAll(offsize,limit) { apis.exam.findAllBySchool(this.school_id,offsize,limit).then(res => { this.showItem=res.data ; console.log('考试一级菜单',res.data) }) },
    ...
    toCreat(){
      ...
      if(this.name !=null && this.condition != null){
        apis.exam.creat(this.name , this.condition ,this.school_id)
        .then(res=>{
    ...
    // 搜索
    search(){
      if(this.searchkey) apis.exam.findAndCountAllLikeByNameSchool(this.searchkey,this.school_id).then(res => { this.showItem=res.data });
      else this.init()
    }
  }
};
