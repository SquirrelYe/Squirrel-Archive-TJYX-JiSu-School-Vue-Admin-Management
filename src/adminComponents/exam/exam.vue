...
export default {
  ...
  methods: {
    ...
    findAndCountAll(offsize, limit) {
      apis.exam.findAllBySchool(this.school_id, offsize, limit).then(res => {
        this.showItem = res.data;
        console.log('考试一级菜单', res.data);
      });
    },
    creat() {
      this.judge = 0;
    ...
    toCreat() {
      ...
      if (this.name != null && this.condition != null) {
        apis.exam.creat(this.name, this.condition, this.school_id).then(res => {
    ...
    toEdit() {
      ...
      if (this.eitem != null) {
        apis.exam.update(this.eitem.id, this.eitem.title, this.eitem.condition).then(res => {
          if (res.data[0]) {
    ...
    DeleteItem(item) {
      if (confirm(`你确定要删除${item.title}？`)) {
        apis.exam.delete(item.id).then(res => {
    ...
    search() {
      if (this.searchkey)
        apis.exam.findAndCountAllLikeByNameSchool(this.searchkey, this.school_id).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    }
