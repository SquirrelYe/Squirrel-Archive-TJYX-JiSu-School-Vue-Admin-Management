...
export default {
  ...
  methods: {
    ...
    // 获取所有考试
    getAllEitem() {
      apis.eitem.findAllBySchool(this.school_id, 0, 1000).then(res => {
        this.eitem = res.data.rows;
        print.log(res.data);
      });
    },
    getTranByExam() {
    ...
    // 查询订单
    getAllEtranByKind(eitem_id) {
      apis.cart.findByExam(eitem_id, this.offsize, this.limit).then(res => {
