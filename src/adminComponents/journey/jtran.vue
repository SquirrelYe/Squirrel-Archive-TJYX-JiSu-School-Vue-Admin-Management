<template>
  <div class="container">
    <div class="row">
      <div class="col-sm-12">
        <h4 class="pull-left page-title">旅游订单列表管理</h4>
      </div>
    </div>
        ...
        <div class="panel panel-default">
          ...
          <div class="row">
            <form class="form-horizontal" role="form">
              <div class="col-md-3">
                <div class="col-md-4 control-label"><strong>旅游项目：</strong></div>
                <div class="col-md-8">
                  <select class="form-control" v-model="jitem_id" @change="getTranByJourney()">
                    <option v-for="(item, index) in jitem" :key="index" :value="item.id">{{ item.name }}</option>
                  </select>
                </div>
              </div>
            </form>
          ...
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <!-- <input type="text" class="form-control search-bar" placeholder="输入搜索旅游订单列表名称" v-model="searchkey" v-on:keyup.enter="search()"> -->
...
export default {
  ...
  methods: {
    ...
    // 获取所有旅游
    getAllJitem() {
      apis.jitem.findAllBySchool(this.school_id, 0, 1000).then(res => {
        this.jitem = res.data.rows;
        print.log(res.data);
      });
    },
    getTranByJourney() {
      this.getAllJtranByKind(this.jitem_id);
      this.offsize = 0;
    },
    // 查询订单
    getAllJtranByKind(jitem_id) {
      apis.cart.findByJourney(jitem_id, this.offsize, this.limit).then(res => {
