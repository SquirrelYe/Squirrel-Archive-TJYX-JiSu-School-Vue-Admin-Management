                  ...
                  <table style="table-layout: fixed" class="table table-bordered table-striped table-hover" id="datatable-editable">
                    ...
                    <tbody v-if="showItem">
                      <tr class="gradeX" v-for="(item, index) in showItem.rows" :key="index" :class="item.condition == -1 ? 'text-danger' : ''">
                        ...
                        <td>JI-{{ item.id }}</td>
                        <td v-if="item.mjourney">{{ item.mjourney.title }}</td>
                        <td v-else></td>
...
export default {
  ...
  methods: {
    ...
    findAndCountAll(offsize, limit) {
      apis.jitem.findAllBySchool(this.school_id, offsize, limit).then(res => {
        this.showItem = res.data;
        console.log('旅游项目', res.data);
      });
    },
    // 获取旅游二级分类
    findAllJourney() {
      apis.mjourney.findAllBySchool(this.school_id, 0, 100).then(res => {
    ...
    toCreat() {
      ...
      if (this.name != null && this.title != null && this.price != null && this.detail != null && this.mjourney_id != null && this.condition != null && this.logo != null) {
        apis.jitem.creat(this.logo, this.name, this.title, this.price, this.detail, this.mjourney_id, this.condition, this.school_id).then(res => {
    ...
    toEdit() {
      ...
      if (this.currentItem != null) {
        ...
        apis.jitem.update(item.id, item.logo, item.name, item.title, item.price, item.detail, item.mjourney_id, item.condition).then(res => {
          if (res.data[0]) {
    ...
    DeleteItem(item) {
      if (confirm(`你确定要删除${item.title}？`)) {
        apis.jitem.delete(item.id).then(res => {
    ...
    search() {
      if (this.searchkey)
        apis.jitem.findAndCountAllLikeByNameSchool(this.searchkey, this.school_id).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    },
