<template>
  <div class="container">
    <div class="row">
      <div class="col-sm-12">
        <h4 class="pull-left page-title">旅游一级菜单管理</h4>
      </div>
    </div>
          ...
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <button class="btn btn-primary waves-effect waves-light" data-toggle="modal" data-target="#Model" @click="creat()">添加旅游一级菜单<i class="fa fa-plus"></i></button>
                <input type="text" class="form-control search-bar" placeholder="输入搜索旅游一级菜单名称" v-model="searchkey" v-on:keyup.enter="search()" />
      ...
      <div class="modal-dialog" style="width: 55%">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel" v-if="judge == 0">新建旅游一级菜单信息</h4>
            <h4 class="modal-title" id="myModalLabel" v-if="judge == 1">修改旅游一级菜单信息</h4>
...
export default {
  ...
  methods: {
    ...
    findAndCountAll(offsize, limit) {
      apis.journey.findAllBySchool(this.school_id, offsize, limit).then(res => {
    ...
    toCreat() {
      ...
      if (this.name != null && this.condition != null) {
        apis.journey.creat(this.name, this.condition, this.school_id).then(res => {
    ...
    toEdit() {
      ...
      if (this.eitem != null) {
        apis.journey.update(this.eitem.id, this.eitem.title, this.eitem.condition).then(res => {
          if (res.data[0]) {
            s_alert.Success('编辑一级菜单成功!', '成功编辑一个菜单旅游一级菜单', 'success');
            this.init();
    ...
    DeleteItem(item) {
      if (confirm(`你确定要删除${item.title}？`)) {
        apis.journey.delete(item.id).then(res => {
          s_alert.Success('删除成功!', '成功移除了一个旅游一级菜单', 'success');
          this.init();
        });
      }
    },
    ...
    search() {
      if (this.searchkey)
        apis.journey.findAndCountAllLikeByNameSchool(this.searchkey, this.school_id).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    }
