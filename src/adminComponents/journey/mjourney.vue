<template>
  <div class="container">
    <div class="row">
      <div class="col-sm-12">
        <h4 class="pull-left page-title">旅游二级菜单管理</h4>
      </div>
    </div>
      ...
      <div class="col-md-12">
        <div class="panel panel-default">
          <div class="panel-heading">
            <h3 class="panel-title">极速校园后台管理系统-旅游管理</h3>
          </div>
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <button class="btn btn-primary waves-effect waves-light" data-toggle="modal" data-target="#Model" @click="creat()">添加旅游二级菜单<i class="fa fa-plus"></i></button>
                <input type="text" class="form-control search-bar" placeholder="输入搜索旅游二级菜单名称" v-model="searchkey" v-on:keyup.enter="search()" />
      ...
      <div class="modal-dialog" style="width: 55%">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel" v-if="judge == 0">新建旅游二级菜单信息</h4>
            <h4 class="modal-title" id="myModalLabel" v-if="judge == 1">修改旅游二级菜单信息</h4>
            <h4 class="modal-title" id="myModalLabel" v-if="judge == 2">查看旅游二级菜单信息</h4>
          </div>
                  ...
                  <div class="panel-body">
                    ...
                    <form class="form-horizontal" role="form" v-if="judge == 1 && eitem">
                      <div class="form-group">
                        <div class="col-md-2 control-label" style="font-weight: 900"><strong>一级菜单:</strong></div>
                        <div class="col-md-10">
                          <select class="form-control" v-model="eitem.journey_id">
                            <option v-for="(item, index) in firstItems" :key="index" :value="item.id">{{ item.title }}</option>
                          </select>
                        </div>
                      </div>
...
export default {
  ...
  methods: {
    ...
    // 获取一级菜单
    findFirstMenu() {
      apis.journey.findAllBySchool(this.school_id, 0, 100).then(res => {
        this.firstItems = res.data.rows;
        console.log('旅游一级菜单', res.data);
      });
    },
    // 获取所有旅游二级菜单
    findAndCountAll(offsize, limit) {
      apis.mjourney.findAllBySchool(this.school_id, offsize, limit).then(res => {
        this.showItem = res.data;
        console.log('旅游二级菜单', res.data);
      });
    },
    creat() {
      this.judge = 0;
    ...
    toCreat() {
      ...
      if (this.first != null && this.name != null && this.condition != null) {
        apis.mjourney.creat(this.first, this.name, this.condition, this.school_id).then(res => {
    ...
    toEdit() {
      ...
      if (this.eitem != null) {
        apis.mjourney.update(this.eitem.id, this.eitem.journey_id, this.eitem.title, this.eitem.condition).then(res => {
          if (res.data[0]) {
            s_alert.Success('编辑二级菜单成功!', '成功编辑一个菜单旅游二级菜单', 'success');
            this.init();
    ...
    DeleteItem(item) {
      if (confirm(`你确定要删除${item.title}？`)) {
        apis.mjourney.delete(item.id).then(res => {
          s_alert.Success('删除成功!', '成功移除了一个旅游二级菜单', 'success');
          this.init();
        });
      }
    },
    ...
    search() {
      if (this.searchkey)
        apis.mjourney.findAndCountAllLikeByNameSchool(this.searchkey, this.school_id).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    }
