...
export default {
  ...
  methods: {
    ...
    findAndCountAll(offsize, limit) {
      apis.school.findAndCountAll(offsize, limit).then(res => {
    ...
    toCreat() {
      ...
      if (this.name != null && this.detail != null && this.province != null) {
        apis.school.creat(this.name, this.detail, this.province).then(res => {
    ...
    toEdit() {
      ...
      if (this.eitem != null) {
        apis.school.update(this.eitem.id, this.eitem.name, this.eitem.detail, this.eitem.province).then(res => {
          if (res.data[0]) {
    ...
    DeleteItem(item) {
      if (confirm(`你确定要删除${item.name}？`)) {
        apis.school.delete(item.id).then(res => {
    ...
    search() {
      if (this.searchkey)
        apis.school.findAndCountAllLikeByName(this.searchkey).then(res => {
          this.showItem = res.data;
        });
      else this.init();
    }
