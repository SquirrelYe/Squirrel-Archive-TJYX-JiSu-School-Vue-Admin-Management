...
export default {
  ...
  methods: {
    ...
    // 获取所有活动信息字典
    findAndCountAll(offsize,limit) { apis.activity.findAllBySchool(this.school_id,offsize,limit).then(res => { this.showItem=res.data }) },
    creat(){ this.judge =0; },
    // 新增活动信息
    toCreat(){
        print.log(this.act)
    ...
    updateCondition(item , c){
        apis.activity.update(item.id ,c)
        .then(res=>{
    ...
    // 搜索
    search(){
      if(this.searchkey) apis.activity.findAndCountAllLikeByTitleSchool(this.searchkey,this.school_id).then(res => { this.showItem=res.data });
      else this.init()
    },
