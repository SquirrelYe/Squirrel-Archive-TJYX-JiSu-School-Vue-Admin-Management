                  ...
                  <table class="table table-bordered table-striped table-hover" style id="datatable-editable">
                    ...
                    <tbody v-if="showItem">
                      <tr class="gradeX" v-for="(item, index) in showItem.rows" :key="index" :class="item.condition == -1 ? 'text-danger' : ''">
                        ...
                        <td>TX-{{ item.id }}</td>
                        <td v-if="item.authen">{{ item.authen.name }}</td>
                        <td v-else></td>
                        <td v-if="item.school">{{ item.school.name }}</td>
                        <td v-else></td>
                        <td v-if="item.authen">{{ item.authen.xuehao }}</td>
                        <td v-else></td>
                        <td v-if="item.authen">{{ item.authen.phone }}</td>
                        <td v-else></td>
                        <td v-if="item.authen">{{ item.authen.mail }}</td>
                        <td v-else></td>
                        <td>102</td>
                        <td>100</td>
...
export default {
  ...
  methods: {
    ...
    findAndCountAll(offsize, limit) {
      apis.tixian.findAllBySchool(this.school_id, offsize, limit).then(res => {
        this.showItem = res.data;
        print.log('现金提现申请', res.data);
      });
    },
    // 资产提现申请
    ...
    search() {
      if (this.searchkey)
        apis.tixian.findAndCountAllLikeByUserNameSchool(this.searchkey, this.school_id, 0, 10).then(res => {
          this.showItem = res.data;
          print.log('搜索', res.data);
        });
      else this.init();
    }
  }
